...
AI_QUICK_MAX_TOKENS = 400   # короткий первый ответ — только итог, без хода решения; 150 обрезал
# на середине слова вопросы с несколькими пунктами/терминами для перечисления (не тест, не
# расчёт) — потолок повышен, но реально расходуется только когда контент того требует: он не
# добавляет токенов к обычному односложному ответу, только снимает обрыв на многопунктных
...
AI_QUICK_SUFFIX = (
    ...
    "перечисления — по одной короткой строке на каждый, без развёрнутых пояснений, но обязательно "
    "закончи перечень до конца, не обрывай на середине.)"
)
