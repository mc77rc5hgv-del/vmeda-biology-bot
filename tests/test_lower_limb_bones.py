...
async def main():
    # 1. topic exists, right place in osteology module ordering, right count
    osteology_topics = list(tb.ANATOMY["module1_osteology"]["topics"].keys())
    assert osteology_topics == [
    ...
    llb = tb.ANATOMY["module1_osteology"]["topics"]["lower_limb_skeleton"]
    assert len(llb["material"]) == 7
    print("lower_limb_skeleton is registered in the right osteology module order: OK")
    ...
    print("anatomy_section:module1_osteology renders: OK")

    cb2 = FakeCB("anatomy_topic:lower_limb_skeleton")
    await tb.cb_anatomy_topic(cb2)
    assert cb2.message.edits, "topic hub should render"
    print("anatomy_topic:lower_limb_skeleton renders: OK")

    cb3 = FakeCB("anatomy_bones:lower_limb_skeleton")
    await tb.cb_anatomy_bones(cb3)
    assert cb3.message.edits, "bones list should render"
    print("anatomy_bones:lower_limb_skeleton renders: OK")

    # 4. every bone hub + its material page render correctly
    for bone in llb["bones_list"]:
        bid = bone["id"]
        cb_hub = FakeCB(f"anatomy_bone_hub:lower_limb_skeleton:{bid}")
        await tb.cb_anatomy_bone_hub(cb_hub)
        assert cb_hub.message.edits, f"bone hub {bid} failed"
        check_html(cb_hub.message.edits[0])

        cb_mat = FakeCB(f"anatomy_bone_material:lower_limb_skeleton:{bid}:0")
    ...
    cb_atlas = FakeCB("anatomy_bone_atlas:lower_limb_skeleton:patella:0")
    ...
    kb_llb = tb.get_anatomy_topic_keyboard("lower_limb_skeleton")
