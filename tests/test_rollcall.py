...
async def main():
    ...
    tb.stats["rollcall_confirmed"]["25-ЛД/СТ-1"] = {"user_id": 1, "confirmed_at": 1}
    assert tb.get_rollcall_confirmed_count() == 0
    assert "25-ЛД/СТ-1" in tb.stats["rollcall_confirmed"], "legacy records must be preserved"

    # 1. Group naming + keyboard shape
    assert tb.rollcall_group_name(1) == "26-ЛД/СТ-1"
    assert tb.rollcall_group_name(40) == "26-ЛД/СТ-40"
    ...
    assert len(kb_texts(menu_kb)) == tb.ROLLCALL_GROUP_COUNT + 1  # 40 groups + back button
    print("40 group buttons + back button, correct callback_data: OK")
    ...
    assert "26-ЛД/СТ-7" in group_text
    ...
    check_html(req_text)
    assert "26-ЛД/СТ-7" in req_text and "@picker7" in req_text and str(picker_uid) in req_text
    ...
    # 7. Admin confirms -> group locked, buyer gets a 30-day bonus, buyer notified
    admin_sent.clear()
    existing_bonus_until = time.time() + 3600
    tb.stats["temporary_access"][str(picker_uid)] = existing_bonus_until
    cb_confirm = FakeCB(confirm_cb_data, uid=ADMIN_ID)
    await tb.cb_rollcall_confirm(cb_confirm)
    assert tb.stats["rollcall_confirmed"]["26-ЛД/СТ-7"]["user_id"] == picker_uid
    expected_expiry = existing_bonus_until + tb.ROLLCALL_BONUS_SECONDS
    ...
    print("admin confirm grants 30-day bonus access, locks the group, notifies the rep: OK")
    ...
    assert any("✅ 26-ЛД/СТ-7" in t for t in texts2)
    ...
    assert cb_group_taken.message.edits and "Перекличка первого курса" in cb_group_taken.message.edits[0][0]
    ...
    tb.stats["rollcall_confirmed"].pop("26-ЛД/СТ-7", None)
    ...
    check_html(ann_text)
    assert "ПЕРЕКЛИЧКА ПЕРВОГО КУРСА" in ann_text
    assert "ПОДПИСКА НА 30 ДНЕЙ" in ann_text
