...
экзаменационный альбом ВМедА пришёл отдельным паком из двух частей (см.
images/operative_surgery/instruments/ и CLAUDE.md) — все 93 позиции по всем 10 группам теперь
сфотографированы. Каждый item — {"name": str, "image": str}; `oh_group_has_photos()` решает по
группе целиком (все позиции с фото или ни одной) — показывать ЧАСТИЧНО заполненный фотоальбом с
необъяснённым пропуском было бы хуже, чем честный текстовый список (`get_oh_instrument_group_text`
остаётся рабочим fallback на случай будущей 11-й группы или исправленной позиции без фото)."""
import json
import math
import os
...
def get_oh_instruments_text() -> str:
    ...
    photo_note = (
        "Все группы уже с реальными фото." if n_with_photos == len(groups)
        else f"{n_with_photos} из {len(groups)} групп уже с реальными фото, остальные добавляются по мере поступления материала."
    )
    return (
        f"🛠 <b>Хирургические инструменты ВМедА</b>\n{tb.DIVIDER}\n\n"
        f"{total} инструментов по {len(groups)} группам — из кафедрального экзаменационного "
        f"альбома. {photo_note}\n\nВыбери группу:"
    )
