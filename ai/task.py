...
class TaskRepresentation:
    ...
    def fingerprint(self) -> str:
        """Ключ для кэша точных совпадений (telegram_bot.get_cached_ai_answer) — нормализует
        текст задания (нижний регистр, только буквы/цифры-слова, пробелы схлопнуты) перед
        хэшированием, чтобы несущественные различия (регистр, пунктуация, лишние пробелы) давали
        один и тот же fingerprint — "Сколько хромосом в соматической клетке человека?" и "сколько
        хромосом в соматической клетке человека" должны совпасть.

        ПОРЯДОК слов при этом сохраняется — раньше слова сортировались перед хэшированием, что
        схлопывало вопросы с одинаковым набором слов, но противоположным смыслом, в одну запись
        кэша: «A вызывает B?» и «B вызывает A?» состоят из одних и тех же слов, но это разные
        вопросы с разными правильными ответами (см. CLAUDE.md/разбор архитектурных недостатков AI-
        режима). Значения условия (values) по-прежнему сортируются по ключу — это независимые
        пары «величина: значение», где порядок объективно не несёт смысла (в отличие от порядка
        слов в самом вопросе), поэтому {"m": "10", "V": "2"} и {"V": "2", "m": "10"} обязаны давать
        один и тот же fingerprint."""
